...
class MetricsCollector {
  constructor() {
    ...
    logger.logInfo("📊 Système de métriques Prometheus initialisé");
  }
  ...
  updateDiscordMetrics(client) {
    ...
    try {
      ...
      logger.logDebug("Métriques Discord mises à jour");
    } catch (error) {
      logger.logError(
  ...
  updateSystemMetrics() {
    try {
      ...
      logger.logDebug("Métriques système mises à jour");
    } catch (error) {
      logger.logError(
  ...
  recordApiRequest(method, endpoint, statusCode, duration) {
    try {
      ...
      logger.logError(
        "Erreur lors de l'enregistrement de la requête API:",
        error
      );
    }
  }
  ...
  recordCommand(commandName, guildId = "dm") {
    try {
      ...
      logger.logError("Erreur lors de l'enregistrement de la commande:", error);
    }
  }
  ...
  recordCacheHit() {
    try {
      ...
      logger.logError("Erreur lors de l'enregistrement du cache hit:", error);
    }
  }
  ...
  recordCacheMiss() {
    try {
      ...
      logger.logError("Erreur lors de l'enregistrement du cache miss:", error);
    }
  }
  ...
  updateStreamStatus(isOnline) {
    try {
      ...
      logger.logError(
        "Erreur lors de la mise à jour du statut du stream:",
        error
      );
    }
  }
  ...
  async getMetrics() {
    try {
      ...
      logger.logError("Erreur lors de la récupération des métriques:", error);
      return "";
    }
  }
  ...
  async getMetricsJson() {
    try {
      ...
      logger.logError(
        "Erreur lors de la récupération des métriques JSON:",
        error
      );
      return [];
    }
