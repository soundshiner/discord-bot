...
class AlertManager {
  constructor() {
    ...
    if (this.webhookUrl) {
      ...
      logger.logInfo("🔔 Système d'alertes webhook initialisé");
    }

    logger.logInfo("🚨 Système d'alertes initialisé");
  }
  ...
  setThresholds(newThresholds) {
    ...
    logger.logInfo("Seuils d'alerte mis à jour:", this.thresholds);
  }
  ...
  createAlert(type, severity, message, data = {}) {
    ...
    logger.logWarn(`🚨 Alerte créée: ${type} - ${severity} - ${message}`);
    return alertId;
  }
  ...
  async sendNotification(alert) {
    try {
      if (!this.webhookClient) {
        logger.logDebug("Webhook non configuré, notification ignorée");
        return;
      }
      ...
      logger.logInfo(`Notification d'alerte envoyée: ${alert.type}`);
    } catch (error) {
      logger.logError("Erreur lors de l'envoi de la notification:", error);
    }
  }
  ...
  async checkMetrics(client) {
    try {
      ...
      logger.logDebug("Vérification des métriques terminée");
    } catch (error) {
      logger.logError("Erreur lors de la vérification des métriques:", error);
      this.createAlert(
        "metrics_check_error",
        "error",
  ...
  checkErrorRate() {
    try {
      ...
      logger.logError(
        "Erreur lors de la vérification du taux d'erreurs:",
        error
      );
    }
  }
  ...
  resolveAlert(alertId) {
    ...
    if (alert) {
      ...
      logger.logInfo(`Alerte résolue: ${alertId}`);
    }
  }
  ...
  cleanupOldAlerts(maxAge = 24 * 60 * 60 * 1000) {
    ...
    if (toDelete.length > 0) {
      logger.logInfo(`${toDelete.length} anciennes alertes supprimées`);
    }
  }
  ...
  recordError(errorType, context = "unknown") {
    try {
      ...
      logger.logError("Erreur lors de l'enregistrement de l'erreur:", error);
    }
  }
}
