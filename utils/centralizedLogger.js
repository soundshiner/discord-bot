...
class CentralizedLogger {
  ...
  async init () {
    try {
      ...
      logger.error(
        'Erreur lors de l\'initialisation du logging centralisé:',
        error
      );
    }
  }
  ...
  addExternalService (service) {
    ...
    const { name } = service;
    logger.info(`Service externe ajouté: ${name}`);
  }
      ...
      for (const file of logFiles) {
        ...
        for (const line of lines) {
          try {
            ...
            // Intentionally empty: ignore malformed lines
          }
        }
      }
    ...
    try {
      ...
      return logs
        .filter((log) => {
          ...
          // Filtre par contenu
          if (query) {
            const searchText = `${log.message} ${JSON.stringify(
              log.meta
            )}`.toLowerCase();
