  ...
  try {
    ...
    if (files.length) {
      loggerInstance.section(
        type === 'command' ? 'commandes' :
        type === 'event' ? 'événements' :
        type === 'task' ? 'tâches' :
        type === 'route' ? 'routes' : 'utilitaires'
      );
    }
        ...
        switch (type) {
          case 'command':
            if (fileModule.default?.data?.name && typeof fileModule.default.execute === 'function') {
              ...
              loggerInstance.custom('CMD', `Commande chargée : ${fileModule.default.data.name}`);
          ...
          case 'event':
            if (fileModule.default?.name && typeof fileModule.default.execute === 'function') {
              ...
              if (fileModule.default.once) client.once(fileModule.default.name, handler);
          ...
          case 'route':
            if (!app) throw new Error('L\'application Express doit être fournie pour charger des routes.');
    ...
    loggerInstance.error(`Erreur générale dans loadFiles pour ${folderName}: ${error.message}`);
    return { loaded: [], failed: [], total: 0 };
  }
}
