...
export default {
  ...
  async execute(interaction) {
    ...
    logger.info(
      `AppState - client: ${client ? "défini" : "undefined"}, db: ${db ? "défini" : "undefined"}, config: ${config ? "défini" : "undefined"}`
    );

    // Utiliser interaction.client comme fallback si AppState.client est undefined
    const discordClient = client || interaction.client;
    const discordConfig = config || (await import("../config.js")).default;
    ...
    try {
        ...
        result = await interactionRetryManager.execute(
          async () => {
            ...
            if (interaction.isButton()) {
              ...
              logger.info(
                `Résultat du bouton: ${JSON.stringify(buttonResult)}`
              );
              return buttonResult;
            }
            ...
            logger.info(
              `Résultat de l'interaction: ${JSON.stringify(interactionResult)}`
            );
      ...
      logger.info(
        `Résultat final après RetryManager: ${JSON.stringify(result)}`
      );
